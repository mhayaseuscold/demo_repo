...
extract(MONTH FROM TH.TASK_LOCK_TIMESTAMP) month,
extract(YEAR FROM TH.TASK_LOCK_TIMESTAMP) YEAR,
CASE WHEN TT_DTL.DETAIL_CODE IN ( 'BP', 'OP', 'PL' ) AND PT_DTL.PICK_SCANNED_TIMESTAMP IS NOT NULL THEN PT_DTL.PICK_SCANNED_TIMESTAMP ELSE TH.TASK_LOCK_TIMESTAMP END AS LOCK_DATETIME,
CASE WHEN TT_DTL.DETAIL_CODE IN ( 'BP', 'OP', 'PL' ) AND PT_DTL.PICK_COMPLETION_TIMESTAMP IS NOT NULL THEN PT_DTL.PICK_COMPLETION_TIMESTAMP ELSE TH.TASK_COMPLETION_TIMESTAMP END AS SCANNED_DATETIME,
...
and year(th.task_lock_timestamp) = RUN_YEAR
AND to_date(TH.TASK_LOCK_TIMESTAMP) > R.EWM_GO_LIVE_DATE
AND (TT_DTL.MASTER_CODE_SYSID = 198 AND TT_DTL.DETAIL_CODE IN ( 'BP', 'PL' )
or (TT_DTL.DETAIL_CODE in ('OP') and TT_DTL.MASTER_CODE_SYSID = 155))
AND datediff(minutes, LOCK_DATETIME, SCANNED_DATETIME)/60 between 0 and 24
AND trim(usr.login_user_name) not in ('AUTOLOC', 'System')
